...
// ********* Internal Measurement Timings *******************************************************************************
  ...
  float Zeitkorr = 1.349;     // ist größer 1.0, kann zur Zeitkorrektur verwendet werden 
...
// ******* Gauge Steuerung und Defaults *********************************************************************************
  ...
  float Brightness = 250;  // Gesamte Helligkeit der LEDs, gültige Werte 1 bis 10 (255)
...
// *** Kalibrierungsfaktor, um die Toleranzen des Eingangsspannungsteilers auszugleichen
  ...
  voltA1 = 0.97708 * 2* 0.00243 * map(inputA1,0,1023,0,2000);   // Faktor 2, da Eingangsteileer 100k + 100k verwendet wird
  voltA2 = 0.96774 * 2* 0.00243 * map(inputA2,0,1023,0,2000);   // Faktor 2, da Eingangsteileer 100k + 100k verwendet wird
  voltA3 = 0.98301 * 2* 0.00243 * map(inputA3,0,1023,0,2000);   // Faktor 2, da Eingangsteileer 100k + 100k verwendet wird
...
// ************ Umschalten intern dBmRSSI auf A1 Voltage  **************************************************************************************************
 ...
 if (Switch_Status < 0.5) {analogWrite(Data_LED_external, Bri_LED); Ext_A1_Gauge();} 
 else {analogWrite(Data_LED_external, 0); dBmRSSI_Gauge();};
...
void Help() {
 ...
 // Serial.flush();
  Serial.println(F(" "));
  Serial.println(F(" "));
  Serial.println(F("********* RSSI & Data Logger *** Verison P2_V2_4_2 (16 bit NeoPixel) ************ "));
  Serial.println(F(" "));
...
*/
  ...
  //   FunktionPeakAlt_Pix = FunktionPeak_Pix;
   ...
   if (counter < 6.3) {Funktion = 0.5 * Anzahl_LED * (1 + cos(counter));};       // ** Start Test und Quittierung von Serial Input
